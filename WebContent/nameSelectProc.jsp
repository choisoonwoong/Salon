...
"c_date, c_memo, rownum as rnum from (select * from company order by c_number) where c_name = ?) where rnum between ? and ?";
...
<%
String sql2 = "select sum(coupon) coupon from coupon where name=?";
...
<%
String sql3 = "select c_name, c_coupon, c_date, c_count from coupon2 where c_name=?";
...
rs.next();
	do {
		...
		String namee = rs.getString("c_name");
...
<%
String sql4 = "select count(*) from company where c_name = ?";
...
<br><br>
<center>
<a href="nameSelectProc.jsp?rnumstart=1&rnumstop=10&name=<%=name%>"><start><<</start></a> &nbsp;
<%
	int start = 1;
	int stop = 10;
	for (pagenum=1; pagenum<=count2/10+0.9; pagenum++) { %>
	<a href="nameSelectProc.jsp?rnumstart=<%=start%>&rnumstop=<%=stop%>&name=<%=name %>"><page><%=pagenum%></page></a> &nbsp;
<%
start += 10;
stop += 10;
} %>

<a href="nameSelectProc.jsp?rnumstart=<%=start-10 %>&rnumstop=<%=stop-10 %>&name=<%=name%>"><stop>>></stop></a>
</center>
